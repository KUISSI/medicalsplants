...
export class AdminLayoutComponent {
  ...
  navGroups: NavGroup[] = [
    ...
    },
    {
      label: 'Recettes',
      items: [
        { path: '/recipes',             label: 'Catalogue',   icon: 'bi-book',         exact: true },
        { path: '/recipes/moderation',  label: 'Modération',  icon: 'bi-shield-check', exact: true }
      ]
    }
  ];
  ...
  private resolveTitleFromUrl(url: string): string {
    ...
    if (url.includes('/recipes/moderation'))      return 'Modération';
