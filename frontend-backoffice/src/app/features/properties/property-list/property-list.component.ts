...
export class PropertyListComponent {
  ...
  searchTerm = '';
  selectedFamily = '';
  families: string[] = [...new Set(MOCK_PROPERTIES.map(p => p.propertyFamily))].sort();
  predefinedFamilies = PREDEFINED_FAMILIES;
  availableSymptoms: Symptom[] = MOCK_SYMPTOMS;
  ...
  applyFilters(): void {
    ...
    if (term) result = result.filter(p => p.title.toLowerCase().includes(term) || p.propertyFamily.toLowerCase().includes(term));
    if (this.selectedFamily) result = result.filter(p => p.propertyFamily === this.selectedFamily);
