...
export class PropertyListComponent implements OnInit {
  ...
  get slideOverTitle(): string {
    return this.editingId ? 'Modifier la propriété' : 'Nouvelle propriété';
  }
  get f() {
    ...
    if (this.editingId) {
      ...
      this.propertyService.update(id, { title, family, description }).subscribe({
        next: (updatedProperty) => {
          ...
          const toRemove = previousSymptomIds.filter(
            (sid) => !this.selectedSymptomIds.includes(sid),
          );
  ...
  applyFilters(): void {
    ...
    if (term)
      result = result.filter(
        (p) => p.title.toLowerCase().includes(term) || p.family.toLowerCase().includes(term),
      );
