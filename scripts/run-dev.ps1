...
$proc = Start-Process -FilePath $javaExe -ArgumentList '-jar', $jar.FullName -NoNewWindow -PassThru -RedirectStandardOutput "$env:TEMP\medicalsplants.log" -RedirectStandardError "$env:TEMP\medicalsplants.log"
Write-Host "PID: $($proc.Id) — streaming logs (ctrl+c to stop)" -ForegroundColor Cyan

# Tail logs
try {
    while (-not $proc.HasExited) {
        Get-Content "$env:TEMP\medicalsplants.log" -Tail 200 -Wait
        Start-Sleep -Seconds 1
    }
    Write-Host "Process exited with code $($proc.ExitCode)" -ForegroundColor Yellow
    Get-Content "$env:TEMP\medicalsplants.log" -Tail 200
}
finally {
